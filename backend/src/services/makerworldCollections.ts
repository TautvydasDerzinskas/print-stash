...
// even unauthenticated -- confirmed against live traffic. No bearer token needed: collection
// listings are public the same way the collection page itself is.
...
async function fetchCollectionJson(url: string): Promise<unknown | null> {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), IMPORT_TIMEOUT_SECONDS * 1000);
  try {
    const res = await fetch(url, { headers: collectionHeaders(), redirect: "follow", signal: controller.signal });
...
export async function fetchMakerworldCollectionTitle(collectionId: string): Promise<string | null> {
  const data = await fetchCollectionJson(`${COLLECTION_API_BASE}/${collectionId}`);
...
export async function fetchMakerworldCollectionEntries(
  ...
  for (;;) {
    ...
    const data = await fetchCollectionJson(url);
