...
export type OpenImportResult = { response: Response; finalUrl: string };
 ...
 * response. Mirrors MakersVault's open_import_response.
 */
export async function openImportResponse(
  ...
  if (isHtmlContentType(contentType)) {
    ...
    return openImportResponse(downloadUrl, body, finalUrl, depth + 1);
  }

  return { response: res, finalUrl };
}
...
export async function downloadImportToTemp(
  url: string,
  body: ImportRequestBody,
): Promise<{ tempPath: string; filename: string; mime: string }> {
  const { response, finalUrl } = await openImportResponse(url, body);
  ...
  return { tempPath, filename, mime };
}

/** Inspects a link without downloading the file body: returns the filename/mime/is_zip it
 * would resolve to. */
export async function inspectImportLink(
  url: string,
  body: ImportRequestBody,
): Promise<{ filename: string; mime: string; is_zip: boolean }> {
  const { response, finalUrl } = await openImportResponse(url, body);
  await response.body?.cancel().catch(() => undefined);
  const filename = buildImportFilename(finalUrl, response.headers, body.filename);
  const mime = mimeFromContentType(response.headers.get("content-type"), filename);
  const isZip = path.extname(filename).toLowerCase() === ".zip";
  return { filename, mime, is_zip: isZip };
}
...
export async function importPrintFromUrl(
  ...
  const { tempPath, filename, mime } = await downloadImportToTemp(url, body);
  ...
  try {
    return await createPrint(meta, path.parse(filename).name, [{ filename, mime, tempFilePath: tempPath }]);
  } finally {
