...
type PrintWithPlatesAndFolder = Print & { plates: Plate[]; folder: Folder | null };

/**
 * Builds zip entries for a set of prints: `{folder_or_unassigned}/{print.name}/{plate.filename}`
 * for every plate, and `.../supporting/{file.filename}` for each print's supporting files.
 */
export async function buildZipEntries(prints: PrintWithPlatesAndFolder[]): Promise<ZipEntryDescriptor[]> {
...
export async function sendPrintsZip(res: Response, prints: PrintWithPlatesAndFolder[], downloadName: string): Promise<void> {
