...
// PrintCard/components rather than being shared.
export default function SupportingFilesPanel({
  ...
  const loadSupportingFiles = async () => {
    ...
    try {
      setSupportingFiles(await listPrintFiles(item.id));
    } catch (err) {
      console.error(err);
  ...
  const addRelatedFiles = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of files) {
        updated = await uploadPrintFile(item.id, file);
  ...
  const removeRelatedFile = async (file: PrintFile) => {
    ...
    try {
      const updated = await deletePrintFile(item.id, file.id);
      onPrintChanged(updated);
