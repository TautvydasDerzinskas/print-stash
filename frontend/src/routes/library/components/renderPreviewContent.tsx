...
import { type Print, type Plate, fileUrl } from "../../../services/api";
...
import { ModelSnapshot } from "./ModelViewer/components/ModelSnapshot";
...
export function renderPreviewContent(
  ...
  const plateUrl = fileUrl(plate.url);
  const thumbUrl = variant === "card"
    ? (print.thumb_url ? fileUrl(print.thumb_url) : null)
    : (plate.thumb_url ? fileUrl(plate.thumb_url) : null);
