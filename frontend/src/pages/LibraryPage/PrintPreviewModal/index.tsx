...
import { UnauthorizedError, LastPlateError } from "../../../api/client";
...
export default function PrintPreviewModal({
  ...
  const handleAddPlateFiles = async (files: File[]) => {
    ...
    setAddingPlate(true);
    try {
      const { print: updated } = await printsApi.addPlates(print.id, files);
  ...
  const openActiveInSlicer = () => {
    ...
    const url = printsApi.fileUrl(isFirstPlate ? (print.slicer_url || activePlate.url) : activePlate.url);
  ...
  const openActiveInEngraving = () => {
    ...
    const url = printsApi.fileUrl(activePlate.url);
      ...
      <DialogActions>
        {activePlate && (
          <Button
            ...
            component="a"
            href={printsApi.fileUrl(activePlate.url)}
