...
export default function LibraryPage({
  ...
  const loadMore = async () => {
    ...
    setLoadingMore(true);
    try {
      const data = await printsApi.list({
        q,
        tags: activeTags,
  ...
  const uploadFlatAsMultiplate = async (files: File[], targetFolderId: string | null) => {
    try {
      const result = await printsApi.upload(files, { folder_id: targetFolderId || undefined, mode: "multiplate" });
  ...
  const onSaveTags = async (id: string, tags: string[]) => {
    ...
    for (const targetId of targets) {
      try {
        await printsApi.setTags(targetId, tags);
      } catch (err) {
        if (err instanceof UnauthorizedError) {
          onUnauthorized?.();
          aborted = true;
  ...
  const onSaveDetails = async (id: string, payload: { notes: string; creator: string; collection: string }) => {
    try {
      await printsApi.updateMeta(id, payload);
      await refresh();
    } catch (err) {
  ...
  const onRename = async (id: string, name: string) => {
    try {
      await printsApi.updateMeta(id, { name });
      await refresh();
    } catch (err) {
  ...
  const onMoveFolder = async (id: string, folder_id: string | null) => {
    try {
      setMovingId(id);
      await printsApi.updateFolder(id, folder_id);
      await refresh();
    } catch (err) {
  ...
  const openInSlicer = (print: Print) => {
    ...
    const plate = print.plates[0];
    const url = printsApi.fileUrl(print.slicer_url || plate?.url || "");
  ...
  const openInEngraving = (print: Print) => {
    ...
    const plate = print.plates[0];
    if (!plate) return;
    const url = printsApi.fileUrl(plate.url);
    const params = new URLSearchParams({
      url,
  ...
  const downloadSelected = async () => {
    ...
    try {
      ...
      const res = await printsApi.downloadZip({ print_ids: Array.from(selectedIds) });
  ...
  const downloadByTag = async (tag: string) => {
    ...
    try {
      ...
      const res = await printsApi.downloadZip({ tag });
  ...
  const removePrint = async (print: Print) => {
    ...
    for (const targetId of targets) {
      try {
        await printsApi.delete(targetId);
      } catch (err) {
        if (err instanceof UnauthorizedError) {
          onUnauthorized?.();
          aborted = true;
