...
func TestRedactURLHidesTokenQueryValue(t *testing.T) {
	redacted := redactURL("https://printstash.example/file?id=asset-1&token=secret-value")
...
func TestHandleProtocolRejectsUntrustedShapesBeforeDownload(t *testing.T) {
	tests := []string{
		"https://printstash.example/file",
		"print-stash://delete?url=https://printstash.example/file",
		"print-stash://open",
		"print-stash://open?url=file:///tmp/model.stl",
	}
