...
// scheme is only for protocols PrintStash can't hand off directly to a slicer's own URL handler
// (Bambu Studio's bambustudio:// only accepts links from an allowlist of domains it trusts, which
// print-stash's own download URLs aren't on). OrcaSlicer and PrusaSlicer accept any HTTP(S) URL
// through their own protocol, so they're launched straight from the browser and never reach here.
const scheme = "print-stash"
...
func main() {
	if len(os.Args) < 2 {
		...
		// print-stash:// open to an already-running app bundle out-of-band as an Apple Event --
...
func printUsage() {
	fmt.Println("Usage:")
	fmt.Println("  print-stash-bridge --install")
	fmt.Println("  print-stash-bridge <print-stash://open?...>")
}
...
func runInstallAndReport() {
	...
	fmt.Printf("Installed PrintStash Bridge at %s\n", target)
	fmt.Println("You can now use \"Open in Bambu Studio\" from PrintStash.")
}
...
func configDir() string {
	...
	if err != nil || dir == "" {
		return filepath.Join(os.TempDir(), "printstash-bridge")
	}
	return filepath.Join(dir, "printstash-bridge")
}
...
func resolveSlicerCommand(id string, cfg Config) (string, []string) {
	...
	envKey := "PRINTSTASH_SLICER_" + strings.ToUpper(strings.ReplaceAll(id, "-", "_"))
