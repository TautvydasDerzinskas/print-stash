...
func installSelf() (string, error) {
	...
	targetPath := filepath.Join(installDir, "print-stash-bridge")
	...
	desktopFile := filepath.Join(desktopDir, "print-stash-bridge.desktop")
	desktop := fmt.Sprintf(
	...
	if xdgMime, err := exec.LookPath("xdg-mime"); err == nil {
		_ = exec.Command(xdgMime, "default", "print-stash-bridge.desktop", "x-scheme-handler/print-stash").Run()
	}
